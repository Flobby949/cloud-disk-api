...
class UserController extends Controller {
  ...
  async reg() {
    const { ctx, app } = this;
    // 参数验证，用户名至少5个字符，最⻓20个字符，密码和确认密码必须一致
    ctx.validate({
      username: { required: true, type: 'string', desc: '用户名', range: { min: 5, max: 20 } },
  ...
  // 登录
  async login() {
    const { ctx, app } = this; // 参数验证
    ctx.validate({ username: { required: true, type: 'string', desc: '用户名' }, password: { required: true, type: 'string', desc: '密码' } });
    // 获取到数据
    const { username, password } = ctx.request.body; // 验证用户是否存在
    let user = await app.model.User.findOne({ where: { username } });
    if (!user) {
      return ctx.apiFail('当前用户不存在');
    }
    this.checkPassword(password, user.password);
    user = JSON.parse(JSON.stringify(user)); // 生成token
    user.token = ctx.getToken(user);
    delete user.password; // 加入缓存中
  ...
  async getSize() {
    const { ctx, service } = this;
    return ctx.apiSuccess({ total_size: ctx.authUser.total_size, used_size: ctx.authUser.used_size });
  }
}
